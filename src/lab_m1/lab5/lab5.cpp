...
void Lab5::Init()
{
    ...
    camera->Set(glm::vec3(0, 2, 7.0f), glm::vec3(0, 1, 0), glm::vec3(0, 1, 0));
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_1) && typeProjection) {
		FOV += 10.0f * deltaTime;
    ...
    if (window->KeyHold(GLFW_KEY_2) && typeProjection) {
		FOV -= 10.0f * deltaTime;
        projectionMatrix = glm::perspective(RADIANS(FOV), window->props.aspectRatio, 0.01f, 200.0f);
	}

    if(window->KeyHold(GLFW_KEY_3) && !typeProjection) {
        top -= 25.0f * deltaTime;
        bottom += 25.0f * deltaTime;
        left += 25.0f * deltaTime;
        right -= 25.0f * deltaTime;
        projectionMatrix = glm::ortho(left, right, bottom, top, zNear, zFar);
    }

    if (window->KeyHold(GLFW_KEY_4) && !typeProjection) {
        top += 25.0f * deltaTime;
        bottom -= 25.0f * deltaTime;
        left -= 25.0f * deltaTime;
        right += 25.0f * deltaTime;
...
void Lab5::OnKeyPress(int key, int mods)
{
    ...
    {   
        typeProjection = 0;
		projectionMatrix = glm::ortho(-5.0f, 5.0f, -5.0f, 5.0f, zNear, zFar);
	}
